...
func getDefaultHeaders() map[string]string {
	return map[string]string{
		"User-Agent": fmt.Sprintf("Qanapi/Go %s", internal.PackageVersion),
	}
}
...
func SubstituteServerVariables(templateURL *url.URL, cfg *RequestConfig) (*url.URL, error) {
	...
	if strings.Count(baseURL, "SUBDOMAIN") >= 1 {
		if cfg.Subdomain == "" {
			return nil, fmt.Errorf("must provide Subdomain to substitute %s", baseURL)
		}
